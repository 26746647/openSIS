...
//			DBQuery('INSERT INTO school_calendars (SYEAR,SCHOOL_ID,TITLE,DEFAULT_CALENDAR,ROLLOVER_ID) SELECT SYEAR+1,SCHOOL_ID,TITLE,DEFAULT_CALENDAR,CALENDAR_ID FROM school_calendars WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\'');
                        DBQuery("INSERT INTO school_calendars (SYEAR,SCHOOL_ID,TITLE,DEFAULT_CALENDAR,ROLLOVER_ID) SELECT SYEAR+1,SCHOOL_ID,CONCAT(TITLE,'_',SYEAR+1),DEFAULT_CALENDAR,CALENDAR_ID FROM school_calendars WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'");
                        //------------------newly added-------------------
...
//                      DBQuery("INSERT INTO attendance_calendar (SYEAR,SCHOOL_ID,SCHOOL_DATE,MINUTES,BLOCK,CALENDAR_ID) SELECT SYEAR+1,SCHOOL_ID,SCHOOL_DATE+INTERVAL '1' YEAR,MINUTES,BLOCK,(SELECT CALENDAR_ID FROM school_calendars WHERE attendance_calendar.CALENDAR_ID=school_calendars.ROLLOVER_ID) AS CAL_ID FROM attendance_calendar WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'");
//                        if($_SESSION['custom_date']=='Y')
//                        roll_given_date ('calendar_events',$calendar['CALENDAR_ID'],$calendar['ROLLOVER_ID']);
//                        else
...
//			DBQuery('DELETE FROM course_periods WHERE SYEAR=\''.$next_syear.'\' AND SCHOOL_ID=\''.UserSchool().'\'');
			...
			foreach($get_cp_dt as $arr_ind=>$arr_dt)
                        ...
                        {
//                            echo 'SELECT PERIOD_ID FROM school_periods  WHERE SYEAR=\''.$arr_dt['SYEAR'].'\' AND ROLLOVER_ID=\''.$cpv_dt['PERIOD_ID'].'\' ';echo '<br>';
