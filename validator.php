...
#***************************************************************************************
	...
	// ------------------------ For Unique Checking ---------------------------------- //
        ...
        {
            case 'pass':
                        $res_pass_chk = mysql_query("SELECT * FROM login_authentication WHERE password = '".md5($_GET['password'])."' AND user_id!='".$_GET['stfid']."' AND profile_id!=0");
            ...
            default :
            case 'pass_o':
                        $res_pass_chk = mysql_query("SELECT * FROM login_authentication WHERE password = '".md5($_GET['password'])."'");
                        $num_pass = mysql_num_rows($res_pass_chk);
                        if($num_pass==0)
                        {
                            echo '1_'.$_GET['opt'];
	...
	#	$result = mysql_query("select username from staff");
		$result = mysql_query("select username from login_authentication");
