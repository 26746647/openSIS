...
//        ORDER BY sp.SORT_ORDER,s.MARKING_PERIOD_ID';
        ...
        AND s.SYEAR = c.SYEAR 
        AND sp.PERIOD_ID = cpv.PERIOD_ID
        AND cp.MARKING_PERIOD_ID IN ('.  implode(',',$mp_ids_arr).')
        AND POSITION(\''.$day.'\' IN cpv.days)>0
        AND s.STUDENT_ID=\''.UserStudentID().'\'
        AND s.SYEAR=\''.UserSyear().'\' 
        AND s.SCHOOL_ID = \''.UserSchool().'\' 
                 ...
                 if(count($week_RET))
                {
                    foreach($wk_schedule_RET as $course)
                    {
                        ...
                        for($j=$today;$j<=$today+$one_day*6;$j=$j+$one_day){
                            ...
                            $day_RET=DBGet(DBQuery('SELECT DISTINCT cp.COURSE_PERIOD_ID,cp.TITLE,cpv.DAYS,r.TITLE AS ROOM FROM course_periods cp,course_period_var cpv,rooms r,marking_periods mp,schedule sch WHERE cp.MARKING_PERIOD_ID=mp.MARKING_PERIOD_ID AND cp.COURSE_PERIOD_ID=sch.COURSE_PERIOD_ID AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND r.ROOM_ID=cpv.ROOM_ID AND
