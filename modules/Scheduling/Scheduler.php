...
//$sql = "SELECT COALESCE(PARENT_ID,COURSE_PERIOD_ID) AS PARENT_ID,COURSE_PERIOD_ID,COURSE_ID,COURSE_ID AS COURSE,GENDER_RESTRICTION,PERIOD_ID,DAYS,TEACHER_ID,MARKING_PERIOD_ID,MP,COALESCE(TOTAL_SEATS,0)-COALESCE(FILLED_SEATS,0) AS AVAILABLE_SEATS,(SELECT COUNT(*) FROM course_periods cp2 WHERE cp2.COURSE_ID=cp.COURSE_ID) AS SECTIONS FROM course_periods cp WHERE cp.SYEAR='".UserSyear()."' AND cp.SCHOOL_ID='".UserSchool()."' ORDER BY SECTIONS,AVAILABLE_SEATS";
 ...
 //$sql = "SELECT COALESCE(cp.PARENT_ID,cp.COURSE_PERIOD_ID) AS PARENT_ID,cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.COURSE_ID AS COURSE,cp.GENDER_RESTRICTION,cp.PERIOD_ID,cp.DAYS,cp.TEACHER_ID,cp.MARKING_PERIOD_ID,cp.MP,COALESCE(cp.TOTAL_SEATS,0)-COALESCE(cp.FILLED_SEATS,0) AS AVAILABLE_SEATS,(SELECT COUNT(*) FROM course_periods cp2 WHERE cp2.COURSE_ID=cp.COURSE_ID) AS SECTIONS FROM course_periods cp,schedule_requests sr WHERE cp.SYEAR='".UserSyear()."' AND cp.SCHOOL_ID='".UserSchool()."' AND cp.MARKING_PERIOD_ID IN (sr.MARKING_PERIOD_ID,'".$parent_id."','".$grand_pid."') ORDER BY SECTIONS,AVAILABLE_SEATS";
  $sql = 'SELECT DISTINCT COALESCE(cp.PARENT_ID,cp.COURSE_PERIOD_ID) AS PARENT_ID,cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.COURSE_ID AS COURSE,cp.GENDER_RESTRICTION,cpv.PERIOD_ID,cpv.DAYS,cp.TEACHER_ID,cp.MARKING_PERIOD_ID,cp.MP,COALESCE(cp.TOTAL_SEATS,0)-COALESCE(cp.FILLED_SEATS,0) AS AVAILABLE_SEATS,(SELECT COUNT(*) FROM course_periods cp2 WHERE cp2.COURSE_ID=cp.COURSE_ID) AS SECTIONS FROM course_periods cp INNER JOIN course_period_var cpv ON cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID LEFT JOIN schedule_requests sr ON cp.COURSE_ID=sr.COURSE_ID WHERE cp.SYEAR=\''.UserSyear().'\' AND cp.SCHOOL_ID=\''.UserSchool().'\'  AND cp.MARKING_PERIOD_ID IN (sr.MARKING_PERIOD_ID,\''.$parent_id.'\',\''.$grand_pid.'\')  ORDER BY SECTIONS,AVAILABLE_SEATS';	
  $cp_parent_RET = DBGet(DBQuery($sql),array(),array('PARENT_ID'));
	$sql = 'SELECT COALESCE(cp.PARENT_ID,cp.COURSE_PERIOD_ID) AS PARENT_ID,cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.COURSE_ID AS COURSE,cp.GENDER_RESTRICTION,cpv.PERIOD_ID,cpv.DAYS,cp.TEACHER_ID,cp.MARKING_PERIOD_ID,cp.MP,COALESCE(cp.TOTAL_SEATS,0)-COALESCE(cp.FILLED_SEATS,0) AS AVAILABLE_SEATS,(SELECT COUNT(*) FROM course_periods cp2 WHERE cp2.COURSE_ID=cp.COURSE_ID) AS SECTIONS FROM course_periods cp,course_period_var cpv WHERE cp.SYEAR=\''.UserSyear().'\' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cp.SCHOOL_ID=\''.UserSchool().'\' AND (cp.PARENT_ID IS NULL OR cp.PARENT_ID=cp.COURSE_PERIOD_ID) ORDER BY SECTIONS,AVAILABLE_SEATS';
	...
	$sql = 'SELECT s.STUDENT_ID,r.REQUEST_ID,s.COURSE_PERIOD_ID,cp.PARENT_ID,s.COURSE_ID,cpv.PERIOD_ID,cp.TEACHER_ID FROM schedule_requests r,schedule s,course_periods cp,course_period_var cpv WHERE
				s.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND cp.PARENT_ID=cp.COURSE_PERIOD_ID AND r.WITH_TEACHER_ID=cp.TEACHER_ID AND cpv.period_id=r.with_period_id AND
				...
				foreach($course_periods as $period_id=>$course_period)
				{
					$scount++;
                                            if($course_period['AVAILABLE_SEATS']!=0)
                                            {
                                                    ...
                                                    $mp_RET = DBGet(DBQuery('SELECT cp.MP,cp.MARKING_PERIOD_ID,cpv.DAYS,cpv.PERIOD_ID,cp.MARKING_PERIOD_ID,cp.TOTAL_SEATS,COALESCE(cp.FILLED_SEATS,0) AS FILLED_SEATS FROM course_periods cp,course_period_var cpv WHERE cp.COURSE_PERIOD_ID=\''.$course_period['COURSE_PERIOD_ID'].'\' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID'));
                                                    ...
                                                    $get_is=DBGet(DBQuery('SELECT IGNORE_SCHEDULING FROM school_periods sp,course_period_var cpv WHERE cpv.PERIOD_ID=sp.PERIOD_ID AND cpv.COURSE_PERIOD_ID=\''.$course_period['COURSE_PERIOD_ID'].'\''));
                                                    if($existing_cp_RET && $get_is[1]['IGNORE_SCHEDULING']!='Y')
                                                        $cp_conflicts=true;
                                                    $period_RET = DBGet(DBQuery('SELECT cpv.DAYS FROM schedule s,course_periods cp,course_period_var cpv,school_periods sp WHERE cp.COURSE_PERIOD_ID=s.COURSE_PERIOD_ID AND cpv.period_id=sp.period_id AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND s.STUDENT_ID=\''.$student_id.'\' AND cpv.PERIOD_ID=\''.$mp_RET[1]['PERIOD_ID'].'\' AND s.MARKING_PERIOD_ID IN ('.$mps.') AND (s.END_DATE IS NULL OR \''.DBDate().'\'<=s.END_DATE) AND sp.ignore_scheduling IS NULL'));
                                                    $days_conflict = false;
                                                    ...
                                                    $per_id = DBGet(DBQuery('SELECT cpv.PERIOD_ID, cpv.DAYS, cp.MARKING_PERIOD_ID FROM course_periods cp,course_period_var cpv WHERE cp.COURSE_PERIOD_ID =\''.$course_per_id.'\' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID'));
                                                    ...
                                                    $child_mpid = $mps;
                                                    $sql = 'SELECT COURSE_PERIOD_ID,START_DATE FROM schedule WHERE STUDENT_ID = \''.$student_id.'\' AND (END_DATE IS NULL OR END_DATE>CURRENT_DATE) AND SCHOOL_ID=\''.UserSchool().'\' AND COURSE_PERIOD_ID NOT IN (SELECT CP.COURSE_PERIOD_ID FROM course_periods CP,school_periods SP,course_period_var CPV WHERE CPV.PERIOD_ID=SP.PERIOD_ID AND CP.COURSE_PERIOD_ID=CPV.COURSE_PERIOD_ID AND SP.IGNORE_SCHEDULING IS NOT NULL)  AND MARKING_PERIOD_ID IN('.$child_mpid.')';
                                                    $xyz = mysql_query($sql);
                                                    ...
                                                    $sel_per_id = DBGet(DBQuery('SELECT cp.COURSE_PERIOD_ID, cpv.PERIOD_ID, cpv.DAYS, cp.MARKING_PERIOD_ID FROM course_periods cp,course_period_var cpv WHERE cp.COURSE_PERIOD_ID = \''.$cp_id.'\' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID'));
