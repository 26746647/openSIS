...
$addr=DBGet(DBQuery('SELECT ADDRESS,STREET,CITY,STATE,ZIPCODE FROM student_address WHERE PEOPLE_ID='.$_SESSION['staff_selected']));
$addr=$addr[1];
echo '<TD colspan=2>';
...
echo TextInput($addr['ADDRESS'],'student_addres[ADDRESS]','','size=25 maxlength=100 id=email class=cell_floating');
echo'</td></tr>';
echo '<tr><td>Street</td><td>:</td><td>';
echo TextInput($addr['STREET'],'student_addres[STREET]','','size=25 maxlength=100 id=email class=cell_floating');
