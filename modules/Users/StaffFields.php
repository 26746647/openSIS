	...
	foreach($_REQUEST['tables'] as $id=>$columns)
	{
		...
		else
		{
			...
			if($table=='staff_fields')
			{
				...
				$id = $id[1]['ID'];
//				$fields = "CATEGORY_ID,CUSTOM_FIELD,";
//				$values = "'".$_REQUEST['category_id']."','staff',";
                                $fields = "CATEGORY_ID,";
...
unset($table);
			...
			elseif($table=='staff_field_categories_for_staff')
			{
				//$id = DBGet(DBQuery("SELECT ".db_seq_nextval('STAFF_FIELD_CATEGORIES_SEQ').' AS ID '.FROM_DUAL));
                                $id = DBGet(DBQuery("SHOW TABLE STATUS LIKE 'staff_field_categories_for_staff'"));
                                $id[1]['ID']= $id[1]['AUTO_INCREMENT'];
				$id = $id[1]['ID'];
                                if($id==5)
                                {
                                    $id++;
                                   DBQuery("ALTER TABLE staff_field_categories_for_staff AUTO_INCREMENT =$id");
                                }
				$fields = "";
				$values ="";
                                
                                $id=DBGet(DBQuery('SELECT MAX(ID) as MAX_ID FROM staff_field_categories_for_staff'));
                                $id=$id[1]['MAX_ID']+1;
				...
				if(User('PROFILE_ID')!='')
					...
					DBQuery("INSERT INTO staff_exceptions (USER_ID,MODNAME,CAN_USE,CAN_EDIT) values('".User('STAFF_ID')."','Users/Staff.php&category_id=$id','Y','Y')");
			}
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='delete')
{
	...
	{
		if(DeletePrompt('staff field category and all fields in the category'))
		{
			...
			DBQuery("DELETE FROM staff_field_categories_for_staff WHERE ID='$_REQUEST[category_id]'");
...
{
	// CATEGORIES
	$sql = "SELECT ID,TITLE,SORT_ORDER FROM staff_field_categories_for_staff ORDER BY SORT_ORDER,TITLE";
	$QI = DBQuery($sql);
	$categories_RET = DBGet($QI);

	if(AllowEdit() && $_REQUEST['id']!='new' && $_REQUEST['category_id']!='new' && ($_REQUEST['id'] || $_REQUEST['category_id']>3))
	...
	{
		$sql = "SELECT CATEGORY_ID,TITLE,TYPE,SELECT_OPTIONS,DEFAULT_SELECTION,SORT_ORDER,REQUIRED,REQUIRED,(SELECT TITLE FROM staff_field_categories_for_staff WHERE ID=CATEGORY_ID) AS CATEGORY_TITLE FROM staff_fields WHERE ID='$_REQUEST[id]'";
	...
	elseif($_REQUEST['category_id'] && $_REQUEST['category_id']!='new' && $_REQUEST['id']!='new')
	{
		$sql = "SELECT TITLE,ADMIN,TEACHER,PARENT,NONE,SORT_ORDER,INCLUDE
				FROM staff_field_categories_for_staff
...
//				$type_options = array('select'=>'Pull-Down','autos'=>'Auto Pull-down','edits'=>'Edit Pull-Down','text'=>'Text');
	...
	{
		echo "<FORM name=F2 id=F2 action=Modules.php?modname=$_REQUEST[modname]&table=STAFF_FIELD_CATEGORIES_FOR_STAFF";
...
//	$link['add']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=new";
	...
	{
//		$sql = "SELECT ID,TITLE,TYPE,SORT_ORDER FROM staff_fields WHERE CATEGORY_ID='".$_REQUEST['category_id']."' AND CUSTOM_FIELD='staff' ORDER BY SORT_ORDER,TITLE";
...
//		ListOutput($fields_RET,$columns,'Staff Field','Staff Fields',$link,array(),$LO_options);
