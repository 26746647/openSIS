...
if($_REQUEST['values'] && ($_POST['values'] || $_REQUEST['ajax'])) {
    foreach($_REQUEST['values'] as $id=>$columns) 
   {
        if(!(isset($columns['TITLE']) && trim($columns['TITLE'])==''))
        {
                        ...
                        foreach($columns as $column=>$value) 
                        {
                            ...
                            { 
                                ...
                                if($samedata[1]['TITLE']!=$value)
                                {
                                    ...
                                    $validate_title=DBGet(DBQuery('SELECT COUNT(1) as TITLE_EX FROM attendance_codes WHERE SYEAR='.UserSyear().' AND SCHOOL_ID='.UserSchool().' AND TITLE=\''.$value.'\''));
         ...
         $validate_title=DBGet(DBQuery('SELECT COUNT(1) as TITLE_EX FROM attendance_codes WHERE SYEAR='.UserSyear().' AND SCHOOL_ID='.UserSchool().' AND TITLE=\''.$title.'\''));
