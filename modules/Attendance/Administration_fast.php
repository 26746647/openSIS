...
if($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax']) && AllowEdit())
{
	foreach($_REQUEST['attendance'] as $student_id=>$values)
	{
		foreach($values as $period=>$columns)
		{
			...
			{
				$period_id = DBGet(DBQuery('SELECT PERIOD_ID FROM course_period_var WHERE COURSE_PERIOD_ID=\''.$period.'\''));
...
if(isset($_REQUEST['student_id']) && optional_param('student_id','',PARAM_ALPHANUM)!='new')
{
	...
	*/
	...
	$schedule_RET = DBGet(DBQuery('SELECT 
										s.STUDENT_ID,concat(c.TITLE, \' - \', s.COURSE_WEIGHT) AS COURSE,cpv.PERIOD_ID,cp.COURSE_PERIOD_ID,p.TITLE AS PERIOD_TITLE,
										\''.''.'\' AS ATTENDANCE_CODE,\''.''.'\' AS ATTENDANCE_TEACHER_CODE,\''.''.'\' AS ATTENDANCE_REASON 
									FROM 
										schedule s,courses c,course_periods cp,course_period_var cpv,school_periods p 
									WHERE 
										s.SYEAR=\''.UserSyear().'\' AND s.SCHOOL_ID=\''.UserSchool().'\' AND cpv.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND s.MARKING_PERIOD_ID IN ('.GetAllMP('QTR',GetCurrentMP('QTR',$date)).')
										AND s.COURSE_ID=c.COURSE_ID 
...
{	global $THIS_RET,$codes_RET,$current_RET,$current_schedule_RET,$date;
	...
	{
		$current_schedule_RET[$THIS_RET['STUDENT_ID']] = DBGet(DBQuery('SELECT cpv.PERIOD_ID,cp.COURSE_PERIOD_ID FROM schedule s,course_periods cp,course_period_var cpv WHERE s.STUDENT_ID=\''.$THIS_RET['STUDENT_ID'].'\' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND s.SYEAR=\''.UserSyear().'\' AND s.SCHOOL_ID=\''.UserSchool().'\' AND cp.COURSE_PERIOD_ID = s.COURSE_PERIOD_ID AND cpv.DOES_ATTENDANCE=\'Y\' AND s.MARKING_PERIOD_ID IN ('.GetAllMP('QTR',GetCurrentMP('QTR',$date)).') AND (\''.$date.'\' BETWEEN s.START_DATE AND s.END_DATE OR s.END_DATE IS NULL)'),array(),array('PERIOD_ID'));
