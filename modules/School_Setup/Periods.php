            ...
            if(!(isset($columns['TITLE']) && trim($columns['TITLE'])==''))
            {
                ...
                ##############################################################################################################
                    if($id!='new')
                    {
                        ...
                        if($title_change!='')
                        {
                        $check_for_cps=DBGet(DBQuery('SELECT COURSE_PERIOD_ID,TITLE FROM course_periods WHERE PERIOD_ID='.$id));
...
{	global $THIS_RET;
 ...
 $ampm_s =date("g:i A", strtotime($row_ampm_s['START_TIME']));
 $f_ampm_s = substr($ampm_s, -2);

 $min_s =date("g:i A", strtotime($row_ampm_s['START_TIME']));
