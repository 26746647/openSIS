...
function CreateList($dli='', $pli='', $sli='', $cli='', $mp='', $mp_name='')
{
   ...
   if($mp!='')
	...
	$sql = 'select
                ...
                (select CONCAT(LAST_NAME,\' \',FIRST_NAME,\' \') from staff where staff_id=course_periods.teacher_id) as teacher, rooms.title as location,days,course_periods.course_period_id,course_periods.begin_date,course_periods.end_date from course_periods,course_period_var,rooms where course_periods.school_id=\''.UserSchool().'\' and course_period_var.room_id=rooms.room_id and course_periods.course_period_id=course_period_var.course_period_id and course_periods.syear=\''.UserSyear().'\' '.$where.'';
