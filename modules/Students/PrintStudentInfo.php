...
//				  UNION SELECT a.ADDRESS_ID,\''.'No Contacts'.'\' AS STUDENT_RELATION,a.ADDRESS,a.STREET,a.CITY,a.STATE,a.ZIPCODE,a.PHONE,a.MAIL_ADDRESS,a.MAIL_STREET,a.MAIL_CITY,a.MAIL_STATE,a.MAIL_ZIPCODE,NULL AS CUSTODY,sja.MAILING,sja.RESIDENCE\''.$address_custom.'\' FROM address a,students_join_address sja                          WHERE a.ADDRESS_ID=sja.ADDRESS_ID AND sja.STUDENT_ID=\''.UserStudentID().'\' AND NOT EXISTS (SELECT \''.''.'\' FROM students_join_people sjp WHERE sjp.STUDENT_ID=sja.STUDENT_ID AND sjp.ADDRESS_ID=a.ADDRESS_ID) ORDER BY ADDRESS ASC,CUSTODY ASC,STUDENT_RELATION'));	
    $addr_sql='SELECT sa.ID AS ADDRESS_ID, sjp.RELATIONSHIP AS STUDENT_RELATION, sa.ADDRESS,sa.STREET,sa.CITY,sa.STATE,sa.ZIPCODE,p.HOME_PHONE AS PHONE,
               (select ADDRESS FROM student_address WHERE student_id='.UserStudentID().' AND TYPE=\'Mail\') as MAIL_ADDRESS,
               (select STREET FROM student_address WHERE student_id='.UserStudentID().' AND TYPE=\'Mail\') as MAIL_STREET,
               ...
               UNION 
               SELECT sa.ID AS ADDRESS_ID,\'No Contacts\' AS STUDENT_RELATION,sa.ADDRESS,sa.STREET,sa.CITY,sa.STATE,sa.ZIPCODE,s.PHONE,
               (select ADDRESS FROM student_address WHERE student_id='.UserStudentID().' AND TYPE=\'Mail\') as MAIL_ADDRESS,
               (select STREET FROM student_address WHERE student_id='.UserStudentID().' AND TYPE=\'Mail\') as MAIL_STREET,
               (select CITY FROM student_address WHERE student_id='.UserStudentID().' AND TYPE=\'Mail\') as MAIL_CITY,
               (select STATE FROM student_address WHERE student_id='.UserStudentID().' AND TYPE=\'Mail\') as MAIL_STATE,
			   ...
			   if(!$categories[1]['RESIDENCE']&&!$categories[1]['MAILING']&&!$categories[1]['BUS'] || $categories[1]['RESIDENCE']=='Y'&&$address['RESIDENCE']=='Y' || $categories[1]['MAILING']=='Y'&&$address['MAILING']=='Y' || $categories[1]['BUS']=='Y'&&($address['BUS_PICKUP']=='Y'||$address['BUS_DROPOFF']=='Y'))
				...
				echo "<table width=100% border=0><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">Primary Emergency Contact</td></tr>";
					...
					#$contacts_RET = DBGet(DBQuery("SELECT p.FIRST_NAME,p.MIDDLE_NAME,p.LAST_NAME,sjp.STUDENT_RELATION,sjp.ADDN_HOME_PHONE,sjp.ADDN_WORK_PHONE,sjp.ADDN_MOBILE_PHONE,sjp.ADDN_EMAIL FROM people p,students_join_people sjp WHERE p.PERSON_ID=sjp.PERSON_ID AND sjp.STUDENT_ID='".UserStudentID()."'"));
                                        $Stu_prim_address=DBGet(DBQuery('SELECT p.FIRST_NAME as PRI_FIRST_NAME,p.LAST_NAME as PRI_LAST_NAME,sa.ADDRESS as PRIM_ADDRESS,sa.STREET as PRIM_STREET,sa.CITY as PRIM_CITY,sa.STATE as PRIM_STATE,sa.ZIPCODE as PRIM_ZIPCODE,sjp.RELATIONSHIP as PRIM_STUDENT_RELATION FROM student_address sa,people p,students_join_people sjp WHERE sa.STUDENT_ID=\''.  UserStudentID().'\' AND sa.PEOPLE_ID=p.STAFF_ID AND sa.TYPE=\'Primary\' AND sjp.PERSON_ID=p.STAFF_ID LIMIT 1'));    
                                        $Stu_sec_address=DBGet(DBQuery('SELECT p.FIRST_NAME as SEC_FIRST_NAME,p.LAST_NAME as SEC_LAST_NAME,sa.ADDRESS as SEC_ADDRESS,sa.STREET as SEC_STREET,sa.CITY as SEC_CITY,sa.STATE as SEC_STATE,sa.ZIPCODE as SEC_ZIPCODE,sjp.RELATIONSHIP as SEC_STUDENT_RELATION  FROM student_address sa,people p,students_join_people sjp WHERE sa.STUDENT_ID=\''.UserStudentID().'\' AND sa.PEOPLE_ID=p.STAFF_ID AND sa.TYPE=\'Secondary\' AND sjp.PERSON_ID=p.STAFF_ID LIMIT 1'));    
...
echo "<tr><td valign=top>";
$info_RET = DBGet(DBQuery("SELECT p.FIRST_NAME,p.MIDDLE_NAME,p.LAST_NAME,sjp.RELATIONSHIP AS STUDENT_RELATION,p.HOME_PHONE AS ADDN_HOME_PHONE,p.WORK_PHONE AS ADDN_WORK_PHONE,p.EMAIL AS ADDN_EMAIL,sa.ADDRESS AS ADDN_ADDRESS,sa.STREET AS ADDN_STREET,sa.CITY AS ADDN_CITY,sa.STATE AS ADDN_STATE,sa.ZIPCODE AS ADDN_ZIPCODE FROM people p,students_join_people sjp,student_address sa WHERE p.STAFF_ID=sjp.PERSON_ID AND sa.STUDENT_ID=sjp.STUDENT_ID AND sjp.PERSON_ID=sa.PEOPLE_ID AND sjp.STUDENT_ID='".UserStudentID()."'"));
