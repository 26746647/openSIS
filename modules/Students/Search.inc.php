...
else
{
	...
	{   
		...
		if(!($_REQUEST['expanded_view']=='true' || $_REQUEST['addr'] || $extra['addr'])){
                    ...
                    $extra['FROM'] = ' INNER JOIN students_join_people sam ON (sam.STUDENT_ID=ssm.STUDENT_ID) INNER JOIN staff pt ON (pt.staff_id=sam.person_id) INNER JOIN staff_school_relationship ssr ON (pt.staff_id=ssr.staff_id) AND ssr.syear=\''.  UserSyear().'\''.$extra['FROM'];
