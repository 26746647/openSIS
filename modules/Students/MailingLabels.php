			...
			foreach($RET as $i=>$addresses)
			{
					...
					if($i<1)
				{
					...
					if($_REQUEST['to_address']=='student')
					{
						...
						{ 
						$Stu_address=DBGet(DBQuery('SELECT ADDRESS,STREET,CITY,STATE,ZIPCODE FROM student_address WHERE STUDENT_ID=\''.$address['STUDENT_ID'].'\' AND TYPE=\'Home Address\' LIMIT 1'));
				...
				elseif($_REQUEST['to_address']=='sec_contact')
					{
						...
						foreach($addresses as $key=>$address)
						{ 
						$Stu_address=DBGet(DBQuery('SELECT p.FIRST_NAME as SEC_FIRST_NAME,p.LAST_NAME as SEC_LAST_NAME,sa.ADDRESS as SEC_ADDRESS,sa.STREET as SEC_STREET,sa.CITY as SEC_CITY,sa.STATE as SEC_STATE,sa.ZIPCODE as SEC_ZIPCODE FROM student_address sa,people p WHERE sa.STUDENT_ID=\''.$address['STUDENT_ID'].'\' AND sa.PEOPLE_ID=p.STAFF_ID AND sa.TYPE=\'Secondary\' LIMIT 1'));
                                                $Stu_address=$Stu_address[1];
						if($cols < 1)
						echo '<tr>';
