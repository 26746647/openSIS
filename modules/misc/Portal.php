...
//$user_event_visibility=DBGet(DBQuery('SELECT COUNT(*) AS VISIBLE FROM calendar_events_visibility WHERE CALENDAR_ID IN (SELECT CALENDAR_ID FROM school_calendars WHERE SCHOOL_ID='.UserSchool().' AND SYEAR='.UserSyear().') AND PROFILE_ID='.$_SESSION['PROFILE_ID'].''));
//$user_event_visibility=$user_event_visibility[1]['VISIBLE'];
$userName=  User('USERNAME');
 $link=array();
    $id=array();
    $arr=array();
//    $qr="select to_user,mail_id,to_cc,to_bcc from msg_inbox where isdraft=0 and mail_read_unread=0";
...
//    else
//        echo "<p><font color=#FF0000><b>You have no unread message</b></font></p>";
}
switch (User('PROFILE'))
{
    case 'admin':
