...
//  if($_REQUEST['process']=='edit' && ($_REQUEST['student_standards'] || $_REQUEST['student_standards_comment'] ))
 if($_REQUEST['process']=='edit' && $_REQUEST['student_standards'])   
  { 
    foreach($_REQUEST['student_standards'] as $key=>$value){   // edit standard grade
...
//////comment insertion/////////////////
//      if($_REQUEST['student_standards_comment'])
//      {  
//         $stand_comment = DBGet(DBQuery('SELECT * FROM student_standards_comment WHERE STUDENT_ID="'.$_REQUEST['student_id'].'" AND COURSE_PERIOD_ID="'.$_REQUEST['course_period_id'].'" AND MARKING_PERIOD_ID="'.UserMP().'" '));
//           if(count($stand_comment)!=0)
//            DBQuery('UPDATE student_standards_comment SET COMMENT="'.$_REQUEST['student_standards_comment'].'" WHERE STUDENT_ID="'.$_REQUEST['student_id'].'" AND COURSE_PERIOD_ID="'.$_REQUEST['course_period_id'].'" AND MARKING_PERIOD_ID="'.UserMP().'"  ');     
//           else
//             DBQuery('INSERT INTO student_standards_comment (STUDENT_ID,COURSE_PERIOD_ID,MARKING_PERIOD_ID,COMMENT) VALUES ("'.$_REQUEST['student_id'].'","'.$_REQUEST['course_period_id'].'","'.UserMP().'","'.$_REQUEST['student_standards_comment'].'") ');
//      }
...
///////////add portion/////////////////////////////
}else if($_REQUEST['process']=='add' && $_REQUEST['student_standards'])
{   
    foreach($_REQUEST['student_standards'] as $key=>$value){
       DBQuery("INSERT INTO student_standards (STUDENT_ID,COURSE_PERIOD_ID,COURSE_STANDARD_ID,GRADE_ID,MARKING_PERIOD_ID)
      VALUES($_REQUEST[student_id],$_REQUEST[course_period_id],$key,'".$value['GRADE_ID']."','".$_REQUEST['mp']."')");
//echo '<SCRIPT language=javascript>opener.document.location = "Modules.php?modname='.$_REQUEST['modname'].'"; window.close();</script>';
...
echo '<SCRIPT language=javascript>opener.document.location = "Modules.php?modname='.$_REQUEST['modname'].'&mp='.$_REQUEST['mp'].'"; window.close();</script>';
////////////////////////////////////////
$_REQUEST['process']='edit';
}
$report_card_grades_ret=DBGet(DBQuery("SELECT ID,TITLE FROM report_card_grades WHERE SYEAR='".UserSyear()."' AND GRADE_SCALE_ID=(SELECT STANDARD_SCALE_ID FROM course_periods WHERE COURSE_PERIOD_ID=$_REQUEST[course_period_id]) AND SCHOOL_ID=".UserSchool()));
foreach($report_card_grades_ret as $value){
$report_card_grades[$value['ID']]=$value['TITLE'];
}
$_openSIS['allow_edit']=true;


$us_standards_ret=DBGet(DBQuery("SELECT cs.ID,us.STANDARD_REF_NO AS TITLE,us.STANDARD_DETAILS AS STANDARD_DETAILS, '' AS REPORT_CARD_GRADE,'' AS COMMENT FROM course_standards cs, us_common_core_standards us WHERE cs.STANDARD_ID=us.STANDARD_ID AND cs.COURSE_ID=".$_REQUEST['course_id']." AND cs.STANDARD_TYPE='us_common_core' " ),array('REPORT_CARD_GRADE'=>'_makeGradeForStandard'));
$school_standards_ret =DBGet(DBQuery("SELECT cs.ID,sss.STANDARD_REF_NO AS TITLE,sss.STANDARD_DETAILS AS STANDARD_DETAILS, '' AS REPORT_CARD_GRADE,'' AS COMMENT FROM course_standards cs, school_specific_standards sss WHERE cs.STANDARD_ID=sss.STANDARD_ID AND cs.COURSE_ID=".$_REQUEST['course_id']." AND cs.STANDARD_TYPE='school_specific' " ),array('REPORT_CARD_GRADE'=>'_makeGradeForStandard')); 
$satnd_ret =  array_merge($us_standards_ret,$school_standards_ret);
$count=1;
foreach ($satnd_ret as $satnd_key => $satnd_value) {
 $standards_ret[$count]=  $satnd_value; 
 $count++;
}
...
echo "<FORM name=F1 id=F1 action=for_window.php?modname=$_REQUEST[modname]&modfunc=enter_standards&process=add&student_id=$_REQUEST[student_id]&course_period_id=$_REQUEST[course_period_id]&course_id=$_REQUEST[course_id]&mp=$_REQUEST[mp] method=POST>";
$stu_standards_columns = array('TITLE'=>'Standards','STANDARD_DETAILS'=>'Standard Details','REPORT_CARD_GRADE'=>'Assign Grade');

 ListOutput($standards_ret,$stu_standards_columns,'Standard','Standards',false,false,array('search'=>false,'sort'=>false));
 echo '<br>';
 //print_r($standards_ret);
 /////////////////cooment portion at add time////////////////////////////////////
// $stu_comment_ret[1]= array('COMMENT'=>TextAreaInput('',"student_standards_comment",'','',''));
// $stu_comment_columns = array('COMMENT'=>'Comment');
// ListOutput($stu_comment_ret,$stu_comment_columns,'','',false,false,array('search'=>false,'sort'=>false));
 ////////////////////////////////////////////////////////////////////////////////////////
 
// echo '<table width="95%" cellspacing="1" cellpadding="6" align="center" class="grid"><tr><td class="subtabs">Comment</td></tr><tr><td class="odd">'.TextAreaInput($value,"student_standards_comment",'','','');
// echo '</td></tr></table>';
if($grade_status !='closed' && $grade_status !='not_set'){
echo "<br><CENTER><INPUT class='btn_medium' type=submit value=Save>&nbsp;<INPUT class='btn_medium' type=button value=Close onclick='window.close();'></CENTER>";
}
echo "</FORM>";
}else if($_REQUEST['process']=='edit'){
$stu_us_standards_ret=DBGet(DBQuery("SELECT ss.ID,ss.COURSE_STANDARD_ID,ss.GRADE_ID,us.STANDARD_DETAILS AS STANDARD_DETAILS,'' AS GRADE,us.STANDARD_REF_NO AS STANDARD FROM student_standards ss,course_standards cs,us_common_core_standards us WHERE cs.STANDARD_ID=us.STANDARD_ID AND cs.ID=ss.COURSE_STANDARD_ID AND ss.STUDENT_ID=$_REQUEST[student_id] AND ss.COURSE_PERIOD_ID=".$_REQUEST['course_period_id']." AND ss.MARKING_PERIOD_ID=".$_REQUEST['mp']." AND cs.STANDARD_TYPE='us_common_core' " ),array('GRADE'=>'_makeGradeForStandard'));
$stu_school_standards_ret=DBGet(DBQuery("SELECT ss.ID,ss.COURSE_STANDARD_ID,ss.GRADE_ID,sss.STANDARD_DETAILS AS STANDARD_DETAILS,'' AS GRADE,sss.STANDARD_REF_NO AS STANDARD FROM student_standards ss,course_standards cs,school_specific_standards sss WHERE cs.STANDARD_ID=sss.STANDARD_ID AND cs.ID=ss.COURSE_STANDARD_ID AND ss.STUDENT_ID=$_REQUEST[student_id] AND ss.COURSE_PERIOD_ID=".$_REQUEST['course_period_id']." AND ss.MARKING_PERIOD_ID=".$_REQUEST['mp']." AND cs.STANDARD_TYPE='school_specific' " ),array('GRADE'=>'_makeGradeForStandard'));
$stu_satnd_ret =  array_merge($stu_us_standards_ret,$stu_school_standards_ret);
$count=1;
foreach ($stu_satnd_ret as $satnd_key => $satnd_value) {
 $students_standards_ret[$count]=  $satnd_value; 
 $count++;
}

$stu_standards_columns = array('STANDARD'=>'Standards','STANDARD_DETAILS'=>'Standard Details','GRADE'=>'Assign Grade');
echo "<FORM name=F1 id=F1 action=for_window.php?modname=$_REQUEST[modname]&modfunc=enter_standards&process=edit&student_id=$_REQUEST[student_id]&course_period_id=$_REQUEST[course_period_id]&course_id=$_REQUEST[course_id]&mp=$_REQUEST[mp] method=POST>";
ListOutput($students_standards_ret,$stu_standards_columns,'Standard','Standards',false,false,array('search'=>false,'sort'=>false));
echo '<br>';
 /////////////////cooment portion at add time////////////////////////////////////
// $stu_comment = DBGet(DBQuery('SELECT COMMENT FROM student_standards_comment WHERE STUDENT_ID="'.$_REQUEST[student_id].'" AND COURSE_PERIOD_ID="'.$_REQUEST['course_period_id'].'" AND MARKING_PERIOD_ID="'.UserMP().'" '));
// $stu_comment_ret[1]= array('COMMENT'=>TextAreaInput($stu_comment[1]['COMMENT'],"student_standards_comment",'','',''));
// $stu_comment_columns = array('COMMENT'=>'Comment');
// ListOutput($stu_comment_ret,$stu_comment_columns,'','',false,false,array('search'=>false,'sort'=>false));
 ////////////////////////////////////////////////////////////////////////////////////////
if($grade_status !='closed' && $grade_status !='not_set'){
...
if($course_RET[1]['USE_STANDARDS']=="Y"){
    $extra['SELECT'] .= ",'' AS STANDARDS";
    $extra['functions'] += array('STANDARDS'=>'_makeStandards');
}
////////////////////////////////////////
...
//function _makeStandardComment()
//{
//    global $THIS_RET,$report_card_grades;
//    return TextAreaInput($value,"student_standards_comment",'','','');
//}
