...
//		';	
 ...
 $sql = 'SELECT DISTINCT s.STAFF_ID,CONCAT(s.LAST_NAME,\', \',s.FIRST_NAME) AS FULL_NAME,sp.TITLE,cp.course_period_id FROM staff s,school_periods sp,course_periods cp
			
WHERE sp.PERIOD_ID = cp.course_period_id AND cp.GRADE_SCALE_ID IS NOT NULL AND cp.TEACHER_ID=s.STAFF_ID 
...
AND cp.MARKING_PERIOD_ID IN ('.GetAllMP($mp_type,$cur_mp).') AND cp.SYEAR=\''.UserSyear().'\' AND cp.SCHOOL_ID=\''.UserSchool().'\' AND s.PROFILE=\'teacher\'
			'.(($_REQUEST['period'])?' AND cp.course_period_id=\''.$_REQUEST[period].'\'':'').'
...
//		';
 ...
 $sql_gradecompleted = 'SELECT DISTINCT s.STAFF_ID,cp.course_period_id FROM staff s,school_periods sp,course_periods cp
