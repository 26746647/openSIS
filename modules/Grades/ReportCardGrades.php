                                ...
                                if(!(isset($columns['TITLE']) && trim($columns['TITLE'])==''))
                               {
                                    ...
                                    $flag=1;
                                     ...
                                     ##############end#####################################    
                                            ...
                                            if($_REQUEST['tab_id']!='new')
                                            {
                                                        $validate_title=DBGet(DBQuery('SELECT COUNT(1) as TITLE_EX FROM report_card_grades WHERE SYEAR='.UserSyear().' AND SCHOOL_ID='.UserSchool().' AND TITLE=\''.$title.'\''));
                                            ...
                                            if($validate_title[1]['TITLE_EX']!=0 && $flag!=0)
                                                     {
                                                  echo "<font color='red'><b>Unable to save data, because title already exists.</b></font>";break;
                                                     }
                                                     else
                                                     {
                                                         DBQuery($sql);
