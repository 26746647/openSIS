...
//    $mail_id=$_REQUEST['mail_id'];
//    $mail_delete="delete from msg_inbox where mail_id='$mail_id'";
...
//         {
//             echo "<a href='for_export.php?modname=Messaging/Inbox.php&search_modfunc=list&next_modname=Messaging/Inbox.php&sql_save_session=true&page=&LO_sort=&LO_direction=&LO_search=&LO_save=1&_openSIS_PDF=true&filename=$img_name[$i]&name=$img3[$i]&modfunc=save'>".$img3[$i]."</a>";
//             
//              echo '<br>&nbsp;&nbsp;&nbsp;<br>';
//             
//         }
...
$id=array();
$arr=array();
//$qr="select istrash,mail_id from msg_inbox UNION select (SELECT username FROM login_authentication WHERE USER_ID=istrash AND PROFILE_ID=".$user_dt['PROFILE_ID'].") as istrash,mail_id from msg_outbox";
...
if($count>0)
 $to_user_id=implode(',',$id);
    ...
    $to_user_id='null';
    echo "<FORM name=sav id=sav action=Modules.php?modname=$_REQUEST[modname]&modfunc=delete method=POST onsubmit=\'return a();\'>";
...
// print_r($trash_info);
//    $trash="select * from msg_inbox where to_user='$userName' and istrash=1 order by(mail_id)desc";
// $trash_info=DBGet(DBQuery($trash));

   	//echo '<CENTER><TABLE><TR><TD valign=top>';
         echo '<div style="overflow:auto; width:820px;">';
        echo "<div id='students' >";
	//$columns = array('ACTION'=>'Action','FROM_USER'=>'FROM','MAIL_SUBJECT'=>'SUBJECT','MAIL_DATETIME'=>'DATE/TIME');
	...
	//$link['remove']['variables'] = array('mail_id'=>'MAIL_ID');
        ...
        //echo '<table align="center" width="94%"><tr><td align="right">'.SubmitButton('Delete','','class=delete_mail onclick=\'formload_ajax("sav");\'').'</td></tr></table>';
        }
        echo "";
