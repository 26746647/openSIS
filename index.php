...
//            $cPwd='remember_me_pwd';
//            setcookie($cName, $username, time()+60*60*24*100, "/");
//             setcookie($cPwd, $password, time()+60*60*24*100, "/");
//        }
//        else 
//        {
//            setcookie('remember_me_name', 'gone', time()-60*60*24*100, "/");	
...
//if($_REQUEST['USERNAME']!='' && $_REQUEST['PASSWORD']!='' && $value_min_id == $value_max_id)
		...
		{
echo "UPDATE staff s ,staff_school_relationship ssr SET IS_DISABLE='Y' WHERE ssr.STAFF_ID=s.STAFF_ID AND s.STAFF_ID='".$_SESSION['STAFF_ID']."' AND ssr.SYEAR='$_SESSION[UserSyear]' AND PROFILE_ID!=1";
//		  DBQuery("UPDATE staff s ,staff_school_relationship ssr SET IS_DISABLE='Y' WHERE ssr.STAFF_ID=s.STAFF_ID AND s.STAFF_ID='".$_SESSION['STAFF_ID']."' AND ssr.SYEAR='$_SESSION[UserSyear]' AND PROFILE_ID!=1");
exit;
		  session_destroy();
...
############################################################################################
      $failed_login = $login_RET[1]['FAILED_LOGIN'];
        ...
        DBQuery("UPDATE login_authentication SET LAST_LOGIN=CURRENT_TIMESTAMP,FAILED_LOGIN=NULL WHERE USER_ID='".$login_RET[1]['STAFF_ID']."' AND PROFILE_ID='".$login_RET[1]['PROFILE_ID']."'");
...
//        $avail_staff=  DBGet(DBQuery("SELECT * FROM login_authentication WHERE UPPER(USERNAME)=UPPER('".optional_param('USERNAME', 0, PARAM_RAW)."')"));
...
echo "UPDATE staff SET IS_DISABLE='Y' WHERE STAFF_ID='".$res[1]['USER_ID']."'";exit;
