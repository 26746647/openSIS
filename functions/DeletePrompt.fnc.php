...
function DeletePromptStaffCert($title,$queryString,$action='delete')
{
   $tmp_REQUEST = $_REQUEST;
        ...
        if(!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel'])
	{
		...
		echo "<CENTER><h4>Are you sure you want to $action that ".(strpos($title,' ')===false? ucwords($title):$title)."?</h4><br><INPUT type=submit class=btn_medium value=OK onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&include=Certification_Info&custom=staff&category_id=4&delete_ok=1\");'>&nbsp;<INPUT type=button class=btn_medium name=delete_cancel value=Cancel onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&include=Certification_Info&custom=staff&category_id=4\");'></CENTER>";
		PopTable('footer');
		return false;
	}
	else
...
function DeletePrompt_CommonCore($title,$action='delete',$location,$sujectId,$courseId)
{
	$tmp_REQUEST = $_REQUEST;
	unset($tmp_REQUEST['delete_ok']);
	...
	if(!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel'])
	{
		echo '<BR>';
                ...
                echo "<CENTER><h4>Are you sure you want to $action that $title?</h4><br><FORM action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST><INPUT type=submit class=btn_medium value=OK>&nbsp;<INPUT type=button class=btn_medium name=delete_cancel value=Cancel onclick='window.location=\"Modules.php?modname=$location&subject_id=$sujectId&course_id=$courseId\"'></FORM></CENTER>";
