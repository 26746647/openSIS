...
//                                echo '<br><br>';
//                                print_r($terms);
//                                if(count($t_in)==1 && strlen($t_in[0])==1)
//                                    unset($terms[$t_in[0]]);
                                unset($t_in);
...
function ListOutputStaffCert($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false)
{
	...
	if($_LIST['output']!=false)
	{
					...
					foreach($column_names as $key=>$value)
					{
					 ...
					 //Here to change the ListOutput Header Colour
					 ...
					 else
					   {
						 ...
						 /*echo '</TABLE>';
						 echo "</TD></TR>";
						 echo "<TR style='width:100px; height:50px'><TD align='left' valign='top'><A><b>" . str_replace(' ','&nbsp;',$value) . "</b></A></TD>";
		...
		if($result_count==0)
		{
			...
			// mab - problem with table closing if not opened above - do same conditional?
			if(($result_count > $num_displayed) || (($options['count'] || $display_zero) && ((($result_count==0 || $display_count==0) && $plural) || ($result_count==0 || $display_count==0))))
				#echo '</TD></TR></TABLE>'; edited  for a zero record
				#echo '<td>'; //edited for Search in parents (user>> associate student with parents)
			if($link['add']['link'] && !isset($_REQUEST['_openSIS_PDF']))
			{
				echo '<table width=120px cellspacing=8 cellpadding=6 ><tr><TD align=left class=lone_add >' . button('add',$link['add']['title'],$link['add']['link']) . '</td></tr></table>';
...
function ListOutputPrint_Institute_Report($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false)
{
	...
	if($_LIST['output']!=false)
	{
		...
		if($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!='Search'))
		{
			...
			for($i=$start;$i<=$stop;$i++)
			{
				...
				elseif($color=='bgcolor="#ffffff"')
					...
					//$color = '#EDF3FE';
                                ...
                                if($_REQUEST['modname']=='Reports/Reports.php?func=Ins_r')
                                {
                                    if(isset($_REQUEST['_openSIS_PDF']) && $count%$repeat_headers==0)
                                    {
                                        echo "<TR>";
			...
			else{	if(isset($_REQUEST['_openSIS_PDF']) && $count%$repeat_headers==0)
				{
					if($count!=0)
					{
						echo '</TABLE>';
...
function ListOutputStaffPrint($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false,$for_window='')
{
	if(!isset($options['save']))
		$options['save'] = true;
	if(!isset($options['print']))
...
function ListOutputExcel($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false,$for_window='')
{
	if(!isset($options['save']))
		$options['save'] = true;
	if(!isset($options['print']))
...
function ListOutputNew($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false,$for_window='')
{
	if(!isset($options['save']))
		$options['save'] = true;
	if(!isset($options['print']))
				...
				if($result_count > $num_displayed)
				{
					...
					if(ceil($result_count/$num_displayed) <= 10)
					{
						for($i=1;$i<=ceil($result_count/$num_displayed);$i++)
						{
							if($i!=$Request_page){
	if($for_window=='for_window'){
						$pages .= "<A HREF=".str_replace('Modules.php', 'for_window.php', $PHP_tmp_SELF)."&LO_sort=$_REQUEST[LO_sort]&LO_direction=$_REQUEST[LO_direction]&LO_searchNew=".urlencode($_REQUEST['LO_searchNew'])."&page=$i&list_type=$singular>$i</A>, ";
						}else{
