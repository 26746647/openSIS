	...
	switch($type)
	{
		...
		break;
			...
			if(!UserStaffID() && $_REQUEST['staff_id']!='new' || $extra['new']==true)
			{
				...
				$_REQUEST['next_modname'] = $_REQUEST['modname'];
				if(!$_REQUEST['modname']) $_REQUEST['modname'] = 'Users/Search.php';
				include('modules/Users/Search_Staff.inc.php');
			}
		break;
	...
	switch($type)
	{
		case 'staff_id':
			...
			if($_REQUEST['staff_id'])
			{
				...
				if($_REQUEST['staff_id']!='new')
				{
					...
					//echo "SELECT PROFILE_ID FROM STAFF S WHERE STAFF_ID='".$_SESSION['staff_id']."'";
					if($pro[1]['PROFILE_ID']==4)
					$_SESSION['fn']='user';
					else
					$_SESSION['fn']='staff';
		...
		case 'general_info':
			...
			foreach($ethnicity_RET as $ethnicity)
					...
					echo '<tr><td align=right width=120>Date of Birth</td><td>'.DateInput('','search[BIRTHDATE]','') .'</td></tr>';
			/*echo "<SELECT name=sex style='max-width:250;'>Gender<OPTION value=''>N/A</OPTION><OPTION value='Male'>Male</OPTION></SELECT>";*/
			/*echo '<tr><td align=right width=120>Address</td><td><input type=text name="addr" size=30 class="cell_floating"></td></tr>';*/
