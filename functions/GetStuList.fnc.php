...
//			$select = ',s.PHONE,s.GENDER,s.ETHNICITY,COALESCE(a.MAIL_ADDRESS,a.ADDRESS) AS ADDRESS,COALESCE(a.MAIL_CITY,a.CITY) AS CITY,COALESCE(a.MAIL_STATE,a.STATE) AS STATE,COALESCE(a.MAIL_ZIPCODE,a.ZIPCODE) AS ZIPCODE ';
                        $select = ',s.PHONE,s.GENDER,s.ETHNICITY,COALESCE((SELECT ADDRESS FROM student_address WHERE student_id=ssm.STUDENT_ID AND TYPE="Mail"),sa.ADDRESS) AS ADDRESS,COALESCE((SELECT CITY FROM student_address WHERE student_id=ssm.STUDENT_ID AND TYPE="Mail"),sa.CITY) AS CITY,COALESCE((SELECT STATE FROM student_address WHERE student_id=ssm.STUDENT_ID AND TYPE="Mail"),sa.STATE) AS STATE,COALESCE((SELECT ZIPCODE FROM student_address WHERE student_id=ssm.STUDENT_ID AND TYPE="Mail"),sa.ZIPCODE) AS ZIPCODE ';
...
//			$extra2['SELECT_ONLY'] = 'ssm.STUDENT_ID,p.STAFF_ID AS PERSON_ID,p.FIRST_NAME,p.LAST_NAME,sjp.RELATIONSHIP AS STUDENT_RELATION,pjc.TITLE,pjc.VALUE,a.PHONE,sjp.ADDRESS_ID ';
	...
	switch(User('PROFILE'))
	{
		case 'admin':
			...
			else
			{
				...
				$sql .= ','.db_case(array('(ssm.SYEAR=\''.UserSyear().'\' AND (ssm.START_DATE IS NOT NULL AND ((\''.date('Y-m-d',strtotime($extra['DATE'])).'\'<=ssm.END_DATE OR ssm.END_DATE IS NULL)) OR ssm.DROP_CODE='.$get_rollover_id.') )','true',"'<FONT color=green>Active</FONT>'","'<FONT color=red>Inactive</FONT>'")).' AS ACTIVE ';
...
{
    global $THIS_RET;
//   echo $sql='SELECT pt.STAFF_ID AS STAFF_ID,CONCAT(pt.LAST_NAME,\' \',pt.FIRST_NAME) AS PARENT FROM students_join_people sju,staff pt, staff_school_relationship ssr  WHERE pt.staff_id=ssr.staff_id AND sju.person_id=pt.staff_id AND sju.student_id=\''.$value.'\' AND ssr.syear=\''.UserSyear().'\'';
