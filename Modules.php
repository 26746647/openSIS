...
$QI = DBQuery("SELECT cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.TITLE,cp.SCHOOL_ID,cpv.PERIOD_ID FROM course_periods cp,course_period_var cpv WHERE cp.SYEAR='".UserSyear()."' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cp.SCHOOL_ID='".UserSchool()."' AND cp.COURSE_ID='".UserCourse()."' AND (TEACHER_ID='".User('STAFF_ID')."' OR SECONDARY_TEACHER_ID='".User('STAFF_ID')."') AND MARKING_PERIOD_ID IN (".GetAllMP($allMP,UserMP()).")");
$RET = DBGet($QI);
	...
	if(count($_openSIS['Menu'][$modcat]))
	{
		...
		elseif(User('PROFILE')=='student' && $modcat=="Scheduling")
		{
			echo "<li><a style='cursor:hand;' HREF=# onmouseup='check_content(\"ajax.php?modname=$modcat/Search.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \"".str_replace('_',' ',$modcat)."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=".$modcat."\"'><span>Schedule</span></a></li>";
                ...
                {                   
                        echo "<li><a style='cursor:hand;' HREF=# onmouseup='check_content(\"ajax.php?modname=$modcat/Inbox.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \"".str_replace('_',' ',$modcat)."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=".$modcat."\"'><span>Messaging</span></a></li>";
                }
		else
		{
